...
class WebsiteForumInherit(WebsiteForum):
    ...
    # Forum overview
    @http.route(['/forum'], type='http', auth="user", website=True, sitemap=True)
    ...
    def questions(self, forum=None, tag=None, page=1, filters='all', my=None, sorting=None, search='', create_uid=False, include_answers=False, **post):
        ...
        if forum.mode == 'unityplan_questions' or forum.mode == 'unityplan_discussions':
            return request.render("unityplan_website_forum.unityplan_forum_index", values)
        else:
            return request.render("website_forum.forum_index", values)

    @http.route(['''/forum/<model("forum.forum"):forum>/faq'''], type='http', auth="user", website=True, sitemap=True)
    ...
    @http.route(['/forum/<model("forum.forum"):forum>/faq/karma'], type='http', auth="user", website=True,
                sitemap=False)
    ...
    @http.route('/forum/get_tags', type='http', auth="user", methods=['GET'], website=True, sitemap=False)
    def tag_read(self, forum_id, query='', limit=25, **post):
        ...
        return request.make_response(json.dumps(data), eaders=[("Content-Type", "application/json")])
    ...
    def tags(self, forum, tag_char='', filters='all', search='', **post):
        ...
        if search:
            ...
            __, details, __ = request.website._search_with_fuzzy(
                'forum_tags_only', search, limit=None, order=order,
    ...
    @http.route(['''/forum/<model("forum.forum"):forum>/<model("forum.post"):question>'''],
                type='http', auth="user", website=True, sitemap=sitemap_forum_post)
    ...
    # Post
    # --------------------------------------------------
    @http.route(['/forum/<model("forum.forum"):forum>/ask'],
    ...
    def forum_post(self, forum, **post):
        ...
        if forum.mode == 'unityplan_questions' or forum.mode == 'unityplan_discussions':
            return request.render("unityplan_website_forum.unityplan_new_question", values)
        else:
    ...
    def post_edit(self, forum, post, **kwargs):
        ...
        if forum.mode == 'unityplan_questions' or forum.mode == 'unityplan_discussions':
            return request.render("unityplan_website_forum.unityplan_edit_post", values)
        else:
    ...
    @http.route('/forum/<model("forum.forum"):forum>/validation_queue',
                type='http', auth="user", website=True)
    ...
    @http.route('/forum/<model("forum.forum"):forum>/flagged_queue',
                type='http', auth="user", website=True)
    ...
    @http.route('/forum/<model("forum.forum"):forum>/offensive_posts',
                type='http', auth="user", website=True)
    ...
    @http.route('/forum/<model("forum.forum"):forum>/closed_posts',
                type='http', auth="user", website=True)
    ...
    @http.route(['/forum/user/<int:user_id>'],
                type='http', auth="user", website=True)
